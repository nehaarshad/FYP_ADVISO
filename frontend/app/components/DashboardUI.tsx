...
import { Search, Bell, ArrowRight, Clock } from "lucide-react";
  ...
  return (
    <header className="h-24 bg-white/80 backdrop-blur-xl border-b border-slate-200 px-10 flex items-center justify-between sticky top-0 z-40">
      ...
      <div className="h-12 w-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-500 hover:bg-[#1e3a5f] hover:text-white cursor-pointer transition-all relative">
        ...
        <div className="absolute top-3 right-3 h-2 w-2 bg-[#FDB813] border-2 border-white rounded-full" />
      </div>
    </header>
  );
}
...
export function StatCard({
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
      <div className="h-10 w-10 bg-slate-50 text-[#1e3a5f] rounded-xl flex items-center justify-center mb-4">
        {icon}
      </div>
...
/**
 * 3. RECENT STUDENT CARD
 * Coordinator only sees basic student information.
 * No StudentProfile navigation.
 */
export function RecentStudentCard({ student }: any) {
  return (
...
/**
 * 4. STUDENT CARD
 * Coordinator only sees student information.
 * No eye icon.
 * No StudentProfile.
...
export function StudentCard({
  ...
  return (
    <div
      className={`bg-white p-6 rounded-[2.5rem] border flex items-center justify-between group transition-all hover:shadow-xl ${
        risk
          ? "border-red-100 hover:border-red-500"
          : "border-slate-100 hover:border-[#FDB813]"
      }`}
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <ArrowRight
          className="text-slate-200 group-hover:text-[#1e3a5f] group-hover:translate-x-1 transition-all"
          size={20}
        />
      </div>
    </div>
