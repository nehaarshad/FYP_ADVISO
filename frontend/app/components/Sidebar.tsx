  ...
  return (
    <aside className="w-72 bg-[#1e3a5f] text-white hidden lg:flex flex-col shadow-2xl sticky top-0 h-screen z-50">
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        {/* SECTION 2: PROFILE & ADVISOR MANAGEMENT */}
        <div className="py-2">
            ...
            <SidebarItem 
              ...
              label="Edit Student" 
              active={activeTab === "edit-student"} 
              onClick={() => setActiveTab("edit-student")} 
            />

            {/* Batch Advisor Edit/Assign (Same as Advisor View) */}
            <SidebarItem 
              icon={<ShieldCheck size={20}/>} 
              label="Batch Advisor" 
              active={activeTab === "edit-advisor"} 
              onClick={() => setActiveTab("edit-advisor")} 
            />
        </div>
      ...
      <div className="p-6">
         <div className={`p-5 rounded-[2rem] border transition-all group ${activeTab === "profile" ? 'bg-white/20 border-white/20 shadow-lg' : 'bg-gradient-to-b from-white/10 to-transparent border-white/5 hover:bg-white/5'}`}>
            <div onClick={() => setActiveTab("profile")} className="flex items-center gap-3 mb-3 cursor-pointer">
              <div className="h-10 w-10 rounded-full bg-[#FDB813] border-2 border-[#1e3a5f] flex items-center justify-center font-black text-[#1e3a5f] text-xs uppercase group-hover:scale-110 transition-transform">AA</div>
              <div>
