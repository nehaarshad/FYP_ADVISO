...
//               <User size={40} className="text-[#1e3a5f] opacity-40" />
//             </div>
...
export const StudentProfile = ({ student, selectedBatch, onBack, onViewTranscript }: StudentProfileProps) => {
  ...
  return (
    // Scrolling remove karne ke liye h-full aur overflow-hidden add kiya gaya hai
    <div className="animate-in fade-in duration-500 h-full max-h-screen flex flex-col overflow-hidden">
      {/* Top Navigation */}
      ...
      {/* Main Container - overflow-hidden taake screen scroll na ho */}
      <div className="flex flex-col lg:flex-row gap-6 items-start flex-1 overflow-hidden pr-2 pb-6">
        
        {/* LEFT COLUMN - Profile Card */}
        <div className="w-full lg:w-[320px] flex flex-col gap-4 shrink-0 h-full">
          <div className="bg-white rounded-[2.5rem] p-15 flex flex-col items-center shadow-sm border border-slate-50">
            <div className="w-16 h-16 bg-slate-50 rounded-[1.8rem] flex items-center justify-center mb-4 shadow-inner border border-slate-100">
              <User size={40} className="text-[#1e3a5f] opacity-40" />
            </div>
            
            <h2 className="text-xl font-black text-[#1e3a5f] uppercase not-italic leading-tight mb-1 text-center tracking-tighter">
              {student.name}
            </h2>
            <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-4 text-center">
              SAP ID: {student.code}
            </p>
            
            <div className={`px-6 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest mb-6 border-b-4 shadow-sm ${
              student.status === 'Regular' 
              ? 'bg-green-100 text-green-900 border-green-300' 
              : 'bg-red-100 text-red-900 border-red-300'       
            }`}>
            ...
            <div className="w-full space-y-2 pt-4 border-t border-slate-50">
              <div className="flex justify-between items-center bg-slate-50/50 p-3 rounded-xl">
                <span className="text-[8px] font-bold text-slate-400 uppercase">Registration</span>
                <span className="text-[10px] font-black text-[#1e3a5f] uppercase tracking-tighter">564576001</span>
              </div>
              <div className="flex justify-between items-center bg-slate-50/50 p-3 rounded-xl">
                <span className="text-[8px] font-bold text-slate-400 uppercase">Batch</span>
                <span className="text-[10px] font-black text-[#1e3a5f]">{selectedBatch}</span>
              </div>
            </div>
          </div>
          ...
          <button 
            onClick={onViewTranscript}
            className="w-full flex items-center justify-between p-4 rounded-[1.8rem] bg-white text-[#1e3a5f] border-2 border-slate-100 hover:border-amber-400 transition-all group shadow-sm"
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-50 shadow-inner group-hover:scale-110 transition-transform">
                <FileText size={18} className="text-amber-500" />
              </div>
              <div className="text-left">
                <span className="block text-[11px] font-black uppercase tracking-tight group-hover:text-[#1e3a5f]">
                  View Transcript
                </span>
                <span className="text-[8px] font-bold uppercase text-slate-400 not-italic">
                  Grade Report
                </span>
              </div>
            </div>
            <ChevronRight size={16} className="text-slate-200 group-hover:text-amber-500 transition-all" />
          </button>
        </div>

        {/* RIGHT COLUMN - Stats & Roadmap */}
        <div className="flex-1 flex flex-col gap-4 min-w-0 w-full overflow-hidden">
          
          {/* STATS BANNER */}
          <div className="bg-[#1e3a5f] rounded-[2rem] p-5 text-white border-b-[6px] border-amber-400 shadow-xl relative overflow-hidden group shrink-0">
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="pl-2">
                <p className="text-[9px] text-amber-400 font-black uppercase tracking-[0.1em] mb-1">
                  Current Standing
                </p>
                <div className="flex items-baseline gap-1">
                  <h2 className="text-3xl font-black text-white ml-2">
                    {student.cgpa || '0.00'}
                  </h2>
              ...
              <div className="bg-white/10 backdrop-blur-md px-6 py-4 rounded-[1.8rem] border border-white/10">
                <p className="text-[8px] font-black text-amber-400 uppercase mb-1 tracking-[0.1em]">
                  Credits
                </p>
                <p className="text-lg font-black text-amber-400"> 
                  85 <span className="text-white"> / 130</span> 
                  <span className="text-[9px] text-slate-400 uppercase ml-1">Hrs</span>
                </p>
              </div>
            </div>
            <BookOpen size={90} className="absolute -right-6 -bottom-6 opacity-10 group-hover:rotate-12 transition-transform duration-700" />
          </div>

{/* ROADMAP AREA - Fixed and Balanced Spacing */}
<div className="bg-white rounded-[2rem] p-13 shadow-sm border border-slate-100 flex flex-col flex-1 min-h-0 overflow-hidden">
  
  {/* Header Section - Fix height and margin */}
  <div className="flex items-center justify-between mb-6 border-b border-slate-50 pb-5 shrink-0">
    <div className="flex items-center gap-4">
      <div className="w-9 h-9 bg-amber-50 rounded-xl flex items-center justify-center text-amber-600 shadow-sm">
        <BookOpen size={18} />
      </div>
      <div>
        <h3 className="text-[13px] font-black uppercase tracking-widest text-[#1e3a5f] not-italic">
          Roadmap Progress
        </h3>
        <p className="text-[8px] font-bold text-slate-400 uppercase">Degree Completion Status</p>
      </div>
    </div>
  </div>

  {/* Roadmap Grid - Ab ye box ke andar fix rahega */}
  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-15 gap-y-5 overflow-y-auto pr-2 custom-scrollbar flex-1">
    {[
      { name: "Computing Core", earned: 30, total: 39, color: "bg-red-500" },
      { name: "Math & Science Foundation", earned: 9, total: 12, color: "bg-rose-300" },
      { name: "General Education", earned: 15, total: 19, color: "bg-slate-400" },
      ...
      return (
        <div key={i} className="group">
          ...
          <div className="w-full h-2.5 bg-slate-50 rounded-full overflow-hidden border border-slate-100 p-[1px] shadow-inner">
            <div 
              className={`h-full rounded-full transition-all duration-1000 ease-out ${cat.color}`} 
              style={{ width: `${percentage}%` }} 
            />
