...
export const SuggestionCard: React.FC<Props> = ({ suggestion, busy, onSchedule }) => (
  <div className="flex items-center justify-between gap-4 p-4 rounded-2xl border border-slate-100 bg-white hover:shadow-md transition-all">
    <div className="flex items-center gap-3.5 min-w-0">
      <div className="w-12 h-12 rounded-xl bg-[#1e3a5f] text-white flex items-center justify-center text-xs font-black shrink-0 shadow-sm">
    ...
    <button
      ...
      disabled={busy}
      className="px-4 py-2 text-xs font-black uppercase tracking-wider text-[#1e3a5f] bg-[#FDB813] hover:bg-[#e5a40f] rounded-xl transition-all shadow-md shadow-[#FDB813]/20 disabled:opacity-50 disabled:shadow-none flex items-center gap-2 shrink-0"
    >
      {busy && (
