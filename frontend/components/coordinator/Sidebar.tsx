...
import { 
  ...
  Info, Database, FileSpreadsheet, FileText 
} from "lucide-react";

export function Sidebar({ 
  activeTab, setActiveTab, showAddUserOptions, setShowAddUserOptions, 
  showRoadmapOptions, setShowRoadmapOptions 
}: any) {
  const router = useRouter(); 

  const handleSignOut = () => {
    // Sahi path as per your folder structure
    ...
    <aside className="w-72 bg-[#1e3a5f] text-white hidden lg:flex flex-col shadow-2xl sticky top-0 h-screen z-50">
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="py-2">
            ...
            <button onClick={() => {setShowRoadmapOptions(!showRoadmapOptions); setShowAddUserOptions(false);}} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all mt-1 ${showRoadmapOptions ? 'bg-white/10' : 'hover:bg-white/5 opacity-70 hover:opacity-100'}`}>
            ...
            <AnimatePresence>
              {showRoadmapOptions && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pl-10 space-y-1 mt-1">
                  <SubMenuItem icon={<Layers size={16}/>} label="CS Department" onClick={() => setActiveTab("cs-roadmaps")} />
                  <SubMenuItem icon={<Map size={16}/>} label="SE Department" onClick={() => setActiveTab("se-roadmaps")} />
                </motion.div>
              )}
            </AnimatePresence>
        </div>
      ...
      <div className="p-6">
         <div 
           className={`p-5 rounded-[2rem] border transition-all group ${activeTab === "profile" ? 'bg-white/20 border-white/20 shadow-lg' : 'bg-gradient-to-b from-white/10 to-transparent border-white/5 hover:bg-white/5'}`}
         >
            <div 
            ...
            >
              ...
              <div>
                ...
                <p className="text-[10px] opacity-50 font-bold uppercase tracking-tighter">Coordinator</p>
              </div>
            </div>
            <button 
