...
export default function SmartAdvisory({
  studentId, studentName, sessionId, selectedBatch, onBack, onSentSuccess,
  ...
  const handleSend = async () => {
    ...
    if (!studentId)             return setLocalError('Student ID is missing.');
    if (!sessionId || sessionId === 0) return setLocalError('Session ID is missing.');
    try {
      const ok = await finalizeRecommendations(studentId, sessionId);
      if (ok) onSentSuccess();
    } catch (err: any) {
  ...
  if (generateError) {
    return (
      <div className="w-full flex flex-col items-center justify-center py-24 px-4">
        ...
        <button onClick={onBack} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-500 hover:text-[#1e3a5f]">
    ...
    <div className="max-w-6xl mx-auto p-2 sm:p-6 space-y-8 bg-gray-50 min-h-screen rounded-2xl shadow-sm">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1 font-medium uppercase tracking-widest">
              {studentName} · Batch: {selectedBatch} | Session ID: {sessionId} | Student ID: {studentId}
            </p>
          </div>
      ...
      <div className="bg-[#1e3a5f] p-6 md:p-8 rounded-2xl text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl border-b-4 border-amber-400">
        <div className="text-center md:text-left">
          <h3 className="text-lg md:text-xl uppercase tracking-tighter font-black">Finalize Recommendation</h3>
    ...
    <div className={`flex items-center gap-2 px-4 py-3 rounded-xl border ${tone}`}>
      <div>
        ...
        <p className="text-lg font-bold">
          ...
          {isOverAllowed && <span className="text-xs text-red-500 block font-black">⚠ Over limit</span>}
        </p>
      </div>
    </div>
  );
