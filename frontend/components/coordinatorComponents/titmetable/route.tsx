  ...
  return (
    <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h2 className="text-2xl font-bold text-[#1e3a5f] uppercase ">Class Timetables</h2>
          <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-widest">Manage Weekly Schedules for {session}</p>
        </div>
        <button className="flex items-center gap-2 px-6 py-3 bg-[#1e3a5f] text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-[#FDB813] transition-all shadow-md">
        ...
        {timetables.map((item) => (
          <div key={item.id} className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 group hover:border-[#1e3a5f]/20 transition-all">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-[7px] font-bold uppercase tracking-widest ${
            ...
            <h4 className="font-bold text-[#1e3a5f] text-md leading-tight uppercase">{item.section}</h4>
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">Type: {item.type}</p>
            <p className="text-[8px] text-slate-300 font-medium uppercase mt-4 ">Updated: {item.lastUpdated}</p>

            <div className="grid grid-cols-2 gap-2 mt-6">
              <button className={`flex items-center justify-center gap-2 py-3 bg-white border border-slate-200 rounded-xl text-[9px] font-bold text-[#1e3a5f] uppercase tracking-widest transition-all ${item.status === 'Uploaded' ? 'hover:bg-slate-100' : 'opacity-40 cursor-not-allowed'}`}>
                <Eye size={12} /> View
              </button>
              <button className="flex items-center justify-center gap-2 py-3 bg-[#1e3a5f] rounded-xl text-[9px] font-bold text-white uppercase tracking-widest hover:bg-[#FDB813] transition-all">
